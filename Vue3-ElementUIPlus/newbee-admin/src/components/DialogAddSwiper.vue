...
export default{
  ...
  setup(props) {
    ...
    const state = reactive({
      uploadImgServer,
      token: localGet('token') || '', // 用于调用上传图片的接口，放在请求头上的token
      ...
      rules: {
        ...
        sort: [
          { required: 'true', message: '排序不能为空', trgger:['change'] }
        ]
      },
      id: ''
    })
    ...
    const handleBeforeUpload = (file) => {
      const sufix = file.name.split('.')[file.name.split('.').length - 1] || ''
      if(!['jpg','jpeg','png'].includes(sufix)){
        ElMessage.error('请上传jpg，jpeg，png 格式图片')
        return false
      }
    }
    // 上传图片
